...
<div class="max-w-7xl mx-auto animate__animated animate__fadeIn">
    ...
    <div class="bg-white dark:bg-emerald-900 rounded-[2.5rem] p-8 shadow-sm border border-emerald-100 dark:border-emerald-800 mb-8">
        ...
        <p class="text-emerald-600 dark:text-emerald-300 mt-2 font-bold text-xs uppercase tracking-widest">
            Pengawasan jadwal konsultasi daring dari seluruh tamu.
        </p>
    </div>
            ...
            <table class="w-full text-left border-collapse min-w-[1000px]">
                ...
                <tbody class="text-sm font-bold text-slate-700 dark:text-slate-200">
                    ...
                    <tr class="border-b border-slate-100 dark:border-slate-700 hover:bg-emerald-50/30 transition-colors">
                        <td class="p-6">
                            <!-- PERBAIKAN: Pengecekan berjenjang agar tidak error jika relasi null -->
                            <span class="block text-emerald-950 dark:text-white font-black">
                                {{ ($item->kunjungan && $item->kunjungan->tamu) 
                                   ? $item->kunjungan->tamu->nama_tamu 
                                   : (\App\Models\Tamu::where('gmail', $item->gmail)->value('nama_tamu') ?? 'Tamu SOWAN') }}
                            </span>
                    ...
                    @empty
                    <tr>
                        <td colspan="5" class="p-12 text-center text-slate-400 font-bold uppercase tracking-widest text-xs">Belum ada data konsultasi.</td>
                    </tr>
                    @endforelse
                </tbody>
            </table>
        </div>
        <div class="p-6 border-t border-slate-100">{{ $konsultasi->links() }}</div>
    </div>
</div>
...
<script>
    function bukaModal(id, aksi) {
        ...
        form.action = `/pimpinan/konsultasi/${id}/proses`; 
