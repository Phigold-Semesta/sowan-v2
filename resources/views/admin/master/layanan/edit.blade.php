...
@section('title', 'Edit Kategori Layanan - ' . $layanan->nama_layanan)
...
<div class="space-y-8 animate__animated animate__fadeIn">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
            <h1 class="text-3xl font-black text-slate-800 dark:text-white tracking-tighter uppercase italic">
                Edit <span class="text-[#008f5d] dark:text-emerald-400">Layanan</span>
            </h1>
            <p class="text-xs font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mt-1">
                Perbarui informasi untuk kategori <span class="text-slate-600 dark:text-slate-300 italic font-black">"{{ $layanan->nama_layanan }}"</span>
            </p>
        </div>
    ...
    <div class="bg-white dark:bg-slate-800 rounded-[2.5rem] p-8 md:p-12 shadow-xl border border-emerald-50 dark:border-slate-700 transition-colors duration-300">
        <form action="{{ route('admin.master.layanan.update', $layanan->id_layanan) }}" method="POST" class="space-y-10">
            @csrf
            @method('PUT')
            ...
            {{-- Action Buttons - Gaya pill-shaped di bagian bawah seperti di gambar --}}
