...
<div class="space-y-8 animate__animated animate__fadeIn">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-6">
        ...
        <div class="relative">
            <h1 class="text-3xl font-black text-slate-800 dark:text-white tracking-tighter uppercase italic">
                Kategori <span class="text-[#008f5d] dark:text-emerald-400">Layanan</span>
            </h1>
        ...
        <div class="flex items-center gap-3 shrink-0">
            ...
            {{-- TOMBOL TAMBAH --}}
            <a href="{{ route('admin.master.layanan.create') }}" 
               class="inline-flex items-center px-6 py-4 bg-[#008f5d] dark:bg-emerald-600 text-white text-[10px] font-black uppercase tracking-widest rounded-2xl hover:bg-emerald-700 dark:hover:bg-emerald-500 hover:shadow-[0_10px_25px_rgba(0,143,93,0.3)] transition-all duration-300 group">
                <i class="fas fa-plus-circle mr-2 group-hover:rotate-90 transition-transform duration-500 text-sm"></i>
                Tambah Layanan
            </a>
        </div>
    </div>
    ...
    <div class="bg-white dark:bg-slate-800 p-5 rounded-[2.5rem] border border-emerald-50 dark:border-slate-700 shadow-sm transition-colors duration-300">
        <form action="{{ route('admin.master.layanan.index') }}" method="GET" class="flex flex-col lg:flex-row gap-4">
            {{-- Search Input --}}
            ...
            <table class="w-full text-left border-collapse">
                <thead>
                    <tr class="bg-slate-50/80 dark:bg-slate-900/50 border-b border-emerald-50 dark:border-slate-700 text-nowrap">
                        ...
                        <th class="px-6 py-6 text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-[0.2em]">Informasi Layanan</th>
                    ...
                    <tr class="hover:bg-emerald-50/40 dark:hover:bg-slate-900/40 transition-all duration-200 group">
                        <td class="px-8 py-5">
                            <span class="text-sm font-black text-slate-400 dark:text-slate-600 font-mono">
                                @if($layanan instanceof \Illuminate\Pagination\LengthAwarePaginator)
                                    {{ str_pad($layanan->firstItem() + $index, 2, '0', STR_PAD_LEFT) }}
                                @else
                        ...
                        <td class="px-6 py-5">
                            <div class="flex flex-col">
                                ...
                                <span class="text-[10px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest mt-1">
                                    ID: <span class="text-emerald-600 dark:text-emerald-500">LAY-{{ str_pad($item->id_layanan, 3, '0', STR_PAD_LEFT) }}</span>
                                </span>
                            </div>
                        </td>
                        ...
                        <td class="px-8 py-5 text-nowrap">
                            <div class="flex justify-center items-center gap-2">
                                ...
                                <form action="{{ route('admin.master.layanan.destroy', $item->id_layanan) }}" 
                                      id="delete-form-{{ $item->id_layanan }}" 
                                      method="POST" 
                                      class="inline">
                                    @csrf
                                    @method('DELETE')
                                    <button type="button" 
                                            title="Hapus Layanan"
                                            onclick="confirmDeleteLayanan('{{ $item->id_layanan }}', '{{ $item->nama_layanan }}')"
                    ...
                    <tr>
                        <td colspan="4" class="px-8 py-24 text-center">
                            <div class="flex flex-col items-center">
                                ...
                                <p class="text-sm font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">Kategori layanan tidak ditemukan</p>
                                <p class="text-xs text-slate-300 dark:text-slate-600 mt-2 italic font-bold">Coba cari dengan kata kunci lain</p>
                            </div>
                        </td>
                    </tr>
                    @endforelse
            ...
            <div class="flex flex-col md:flex-row justify-between items-center gap-6">
                <div class="flex items-center gap-3">
                    ...
                    <p class="text-[10px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest leading-none">
                        @if($layanan instanceof \Illuminate\Pagination\LengthAwarePaginator)
                            Tampil {{ $layanan->firstItem() }} - {{ $layanan->lastItem() }} dari {{ $layanan->total() }} Layanan
                        @else
                            Menampilkan semua {{ $layanan->count() }} Layanan Terdaftar
                        @endif
                    </p>
                </div>
                <div class="custom-pagination">
                    @if($layanan instanceof \Illuminate\Pagination\LengthAwarePaginator)
                        {{ $layanan->appends(request()->query())->links('pagination::tailwind') }}
                    @endif
                </div>
            </div>
        </div>
    ...
    <div class="mt-6 flex items-center gap-4 px-8 py-5 bg-white dark:bg-slate-800 border border-emerald-50 dark:border-slate-700 rounded-[2rem] shadow-sm transition-colors duration-300">
        <div class="shrink-0 w-10 h-10 rounded-2xl bg-emerald-50 dark:bg-emerald-900/30 flex items-center justify-center shadow-inner">
            <i class="fas fa-database text-[#008f5d] dark:text-emerald-400 text-sm"></i>
        </div>
        <div class="flex flex-col">
            <p class="text-[10px] font-black text-slate-800 dark:text-slate-200 uppercase tracking-[0.15em]">Sinkronisasi Sistem</p>
            <p class="text-[9px] font-bold text-slate-400 dark:text-slate-500 uppercase tracking-widest leading-relaxed">
                Data ini terhubung langsung dengan <span class="text-[#008f5d] dark:text-emerald-400 italic font-black">Database LPSE Karawang V2</span>. Perubahan akan berdampak pada form pendaftaran tamu.
            </p>
        </div>
    </div>
</div>
...
<script>
    function confirmDeleteLayanan(id, name) {
        ...
        Swal.fire({
            title: 'HAPUS LAYANAN?',
            html: `Anda akan menghapus kategori <b>${name}</b>.<br><p class="mt-2 text-xs text-red-500 font-bold uppercase tracking-tight italic">Tamu tidak akan bisa memilih layanan ini lagi!</p>`,
            icon: 'warning',
